...
| 压缩比 | WikiText-2 论文 | **WikiText-2 复现** | C4 论文 | **C4 复现** |
|--------|----------------|---------------------|---------|-------------|
| 0% | 5.68 | **5.67** | 7.34 | **7.20** |
| 20% | 7.94 | **7.84** | 15.84 | **15.65** |
| 40% | 13.73 | **13.17** | 75.42 | **50.05** |
| 60% | 66.62 | **58.98** | 471.83 | **378.80** |
| 80% | 1349 | **660.67** | 6224 | **2641.62** |

所有压缩率上 PPL 均优于论文报告值。低压缩率 (20%) 差异极小，高压缩率差距较大，可能与 C4 评估方式 / tokenizer 版本差异有关。

### 下游任务对比 (Accuracy ↑)

论文仅报告 20% 压缩比的下游任务结果，以下为各压缩率的论文 vs 复现对比：

| 任务 | 论文 (20%) | **复现 20%** | **复现 40%** | **复现 60%** | **复现 80%** |
|------|-----------|-------------|-------------|-------------|-------------|
| OpenbookQA | 0.31 | **0.266** | 0.200 | 0.132 | 0.130 |
| ARC_easy | 0.62 | **0.641** | 0.457 | 0.301 | 0.261 |
| WinoGrande | 0.61 | **0.664** | 0.575 | 0.527 | 0.480 |
| HellaSwag | 0.45 | **0.437** | 0.330 | 0.273 | 0.260 |
| PIQA | 0.71 | **0.690** | 0.609 | 0.544 | 0.523 |
| MathQA | 0.21 | **0.239** | 0.220 | 0.218 | 0.205 |
| Avg (6 tasks) | 0.49 | **0.490** | 0.399 | 0.333 | 0.310 |
| TruthfulQA† | 0.26 | **0.390** | 0.433 | 0.476 | 0.501 |
| GSM8K | 0.05 | **0.006** | 0.000 | 0.000 | 0.000 |

> † TruthfulQA: 论文使用 BLEU 指标，复现使用 MC2 指标，不可直接对比。

**分析:**
- **20% 压缩比:** 6 个 MC 任务平均 Accuracy 0.490 vs 论文 0.49，差异在 ±0.05 以内，属于 lm_eval 版本 / prompt 模板差异
- **高压缩率退化:** 40%→80% 压缩率下 Accuracy 持续下降，80% 时多数任务接近随机水平
- **GSM8K:** 论文值仅 0.05，压缩后数学推理能力基本丧失
