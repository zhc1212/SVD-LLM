...
| 任务 | SVD-LLM(W) 论文 | **SVD-LLM(W) 复现 20%** | **复现 40%** | **复现 60%** | **复现 80%** | SVD-LLM 论文 | **SVD-LLM 复现** |
|------|----------------|------------------------|-------------|-------------|-------------|-------------|-----------------|
| OpenbookQA | 0.31 | **0.266** | 0.200 | 0.132 | 0.130 | 0.33 | — |
| ARC_easy | 0.62 | **0.641** | 0.457 | 0.301 | 0.261 | 0.67 | — |
| WinoGrande | 0.61 | **0.664** | 0.575 | 0.527 | 0.480 | 0.69 | — |
| HellaSwag | 0.45 | **0.437** | 0.330 | 0.273 | 0.260 | 0.55 | — |
| PIQA | 0.71 | **0.690** | 0.609 | 0.544 | 0.523 | 0.79 | — |
| MathQA | 0.21 | **0.239** | 0.220 | 0.218 | 0.205 | 0.26 | — |
| Avg (6 tasks) | 0.49 | **0.490** | 0.399 | 0.333 | 0.310 | 0.55 | — |
| TruthfulQA† | 0.26 | **0.390** | 0.433 | 0.476 | 0.501 | 0.28 | — |
| GSM8K | 0.05 | **0.006** | 0.000 | 0.000 | 0.000 | 0.08 | — |

> † TruthfulQA: 论文使用 BLEU 指标，复现使用 MC2 指标，不可直接对比。

**分析:**
- **SVD-LLM(W) 20%:** 6 个 MC 任务平均 Accuracy 0.490 vs 论文 0.49，差异在 ±0.05 以内，属于 lm_eval 版本 / prompt 模板差异
- **高压缩率退化:** 40%→80% 压缩率下 Accuracy 持续下降，80% 时多数任务接近随机水平
- **SVD-LLM vs SVD-LLM(W):** 论文中 LoRA 微调后 Avg 从 0.49 提升至 0.55，待 Phase 2 复现验证
- **MathQA:** 通过本地 Parquet 数据集解决 datasets 4.1.1 兼容性问题

### 复现结论

1. **PPL 复现成功** — SVD-LLM(W) 趋势完全一致，所有压缩率上复现值均优于论文
2. **Downstream 基本一致** — 6 个 MC 任务平均 Accuracy 0.490 vs 论文 0.49，差异在 ±0.05 以内
