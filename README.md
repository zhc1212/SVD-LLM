...
| 压缩比 | 数据集 | SVD-LLM(W) 论文 | **SVD-LLM(W) 复现** | SVD-LLM 论文 | **SVD-LLM 复现** |
|--------|--------|----------------|---------------------|-------------|-----------------|
| 0% | WikiText-2 | 5.68 | **5.67** | 5.68 | — |
| 0% | C4 | 7.34 | **7.20** | 7.34 | — |
| 20% | WikiText-2 | 7.94 | **7.84** | 7.73 | — |
| 20% | C4 | 15.84 | **15.65** | 12.23 | — |
| 40% | WikiText-2 | 13.73 | **13.17** | 9.27 | — |
| 40% | C4 | 75.42 | **50.05** | 15.63 | — |
| 60% | WikiText-2 | 66.62 | **58.98** | 15.00 | — |
| 60% | C4 | 471.83 | **378.80** | 26.26 | — |
| 80% | WikiText-2 | 1349 | **660.67** | 31.79 | — |
| 80% | C4 | 6224 | **2641.62** | 43.71 | — |

SVD-LLM(W) 所有压缩率上 PPL 均优于论文报告值。低压缩率 (20%) 差异极小，高压缩率差距较大，可能与 C4 评估方式 / tokenizer 版本差异有关。SVD-LLM（含 LoRA 微调）待 Phase 2 完成后填入。

### 下游任务对比 (Accuracy ↑)

#### 20% 压缩比

| 任务 | SVD-LLM(W) 论文 | **SVD-LLM(W) 复现** | SVD-LLM 论文 | **SVD-LLM 复现** |
|------|----------------|---------------------|-------------|-----------------|
| OpenbookQA | 0.31 | **0.266** | 0.33 | — |
| ARC_easy | 0.62 | **0.641** | 0.67 | — |
| WinoGrande | 0.61 | **0.664** | 0.69 | — |
| HellaSwag | 0.45 | **0.437** | 0.55 | — |
| PIQA | 0.71 | **0.690** | 0.79 | — |
| MathQA | 0.21 | **0.239** | 0.26 | — |
| Avg (6 tasks) | 0.49 | **0.490** | 0.55 | — |
| TruthfulQA† | 0.26 | **0.390** | 0.28 | — |
| GSM8K | 0.05 | **0.006** | 0.08 | — |

#### 40% 压缩比

| 任务 | SVD-LLM(W) 论文 | **SVD-LLM(W) 复现** | SVD-LLM 论文 | **SVD-LLM 复现** |
|------|----------------|---------------------|-------------|-----------------|
| OpenbookQA | 0.25 | **0.200** | 0.29 | — |
| ARC_easy | 0.33 | **0.457** | 0.59 | — |
| WinoGrande | 0.55 | **0.575** | 0.68 | — |
| HellaSwag | 0.40 | **0.330** | 0.52 | — |
| PIQA | 0.63 | **0.609** | 0.69 | — |
| MathQA | 0.12 | **0.220** | 0.20 | — |
| Avg (6 tasks) | 0.38 | **0.399** | 0.50 | — |
| TruthfulQA† | 0.17 | **0.433** | 0.24 | — |
| GSM8K | 0.02 | **0.000** | 0.07 | — |

#### 60% 压缩比

| 任务 | SVD-LLM(W) 论文 | **SVD-LLM(W) 复现** | SVD-LLM 论文 | **SVD-LLM 复现** |
|------|----------------|---------------------|-------------|-----------------|
| OpenbookQA | 0.10 | **0.132** | 0.18 | — |
| ARC_easy | 0.05 | **0.301** | 0.42 | — |
| WinoGrande | 0.17 | **0.527** | 0.44 | — |
| HellaSwag | 0.10 | **0.273** | 0.31 | — |
| PIQA | 0.21 | **0.544** | 0.35 | — |
| MathQA | 0.04 | **0.218** | 0.12 | — |
| Avg (6 tasks) | 0.11 | **0.333** | 0.30 | — |
| TruthfulQA† | 0.01 | **0.476** | 0.14 | — |
| GSM8K | 0.00 | **0.000** | 0.04 | — |

#### 80% 压缩比

| 任务 | SVD-LLM(W) 论文 | **SVD-LLM(W) 复现** | SVD-LLM 论文 | **SVD-LLM 复现** |
|------|----------------|---------------------|-------------|-----------------|
| OpenbookQA | 0.07 | **0.130** | 0.11 | — |
| ARC_easy | 0.03 | **0.261** | 0.23 | — |
| WinoGrande | 0.04 | **0.480** | 0.21 | — |
| HellaSwag | 0.02 | **0.260** | 0.14 | — |
| PIQA | 0.07 | **0.523** | 0.17 | — |
| MathQA | 0.01 | **0.205** | 0.08 | — |
| Avg (6 tasks) | 0.04 | **0.310** | 0.16 | — |
| TruthfulQA† | 0.00 | **0.501** | 0.04 | — |
| GSM8K | 0.00 | **0.000** | 0.02 | — |

> † TruthfulQA: 论文使用 BLEU 指标，复现使用 MC2 指标，不可直接对比。

**分析:**
- **20% 压缩比:** SVD-LLM(W) 6 个 MC 任务平均 Accuracy 0.490 vs 论文 0.49，差异在 ±0.05 以内
- **40% 压缩比:** 复现 Avg 0.399 优于论文 0.38，ARC_easy / MathQA 复现显著优于论文
- **60%/80% 压缩比:** 论文报告近乎随机 (Avg 0.11/0.04)，但复现仍保持 0.333/0.310，复现显著优于论文
- **SVD-LLM:** LoRA 微调后论文各压缩率均有显著提升，待 Phase 2 复现验证
- **MathQA:** 通过本地 Parquet 数据集解决 datasets 4.1.1 兼容性问题

### 复现结论

1. **PPL 复现成功** — SVD-LLM(W) 趋势完全一致，所有压缩率上复现值均优于论文
2. **Downstream 复现优于论文** — 尤其在高压缩率 (60%/80%) 下复现结果显著优于论文报告值
3. **高压缩率退化明显** — 虽然复现优于论文，60%/80% 仍有明显退化，需要 Phase 2 (Sequential LoRA) 恢复质量
4. **SVD-LLM 待完成** — Phase 2 将实现 Sequential LoRA 微调，填入上表 SVD-LLM 复现列
